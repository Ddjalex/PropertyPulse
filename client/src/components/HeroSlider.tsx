...
export default function HeroSlider() {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center text-white overflow-hidden">
        ...
        <div className="animate-fade-in">
          <h1 className="text-5xl md:text-6xl font-bold mb-6" data-testid="hero-title">
            {heroSlides[currentSlide].title.split(' ').map((word, index) => 
              word === 'Ethiopia' ? (
                <span key={index} className="text-secondary">{word} </span>
              ) : (
          ...
          {heroSlides[currentSlide].cta && (
            <Button 
              size="lg" 
              className="bg-secondary hover:bg-secondary/90 text-secondary-foreground px-8 py-6 text-lg font-medium"
      ...
      {/* Slide Indicators */}
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-2" data-testid="hero-indicators">
